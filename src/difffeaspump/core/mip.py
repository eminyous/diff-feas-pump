...
class MIP(gp.Model):
    ...
    def _get_Ab_sparse(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        indices = torch.tensor(
            [A_rows, A_cols], dtype=torch.long, device=self.device
        )
        values = torch.tensor(A_values, dtype=self.dtype, device=self.device)
...
def read(path: str | Path, env: gp.Env | None = None) -> MIP:
